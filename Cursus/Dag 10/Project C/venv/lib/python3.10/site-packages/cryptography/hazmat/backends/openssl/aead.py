...
def _aead_cipher_name(cipher: "_AEAD_TYPES") -> bytes:
    ...
    if isinstance(cipher, ChaCha20Poly1305):
        ...
        return f"aes-{len(cipher._substitution) * 8}-ccm".encode("ascii")
    elif isinstance(cipher, AESOCB3):
        return f"aes-{len(cipher._substitution) * 8}-ocb".encode("ascii")
    elif isinstance(cipher, AESSIV):
        return f"aes-{len(cipher._substitution) * 8 // 2}-siv".encode("ascii")
    else:
        assert isinstance(cipher, AESGCM)
        return f"aes-{len(cipher._substitution) * 8}-gcm".encode("ascii")
...
def _decrypt(
    ...
    if ctx is None:
        cipher_name = _aead_cipher_name(cipher)
        ctx = _aead_setup(
            backend, cipher_name, cipher._substitution, nonce, tag, tag_length, _DECRYPT
        )
    else:
